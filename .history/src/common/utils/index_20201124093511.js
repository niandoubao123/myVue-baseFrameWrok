...
Vue.filter('formatSeverityValue', function (val) {
  let value
  switch(val){
    case '0':
    value = '不严重'
...
Vue.filter('formatPriorityValue', function (val) {
  let value
  switch(val){
    case 'D':
    value = 'D级'
...
Vue.filter('isNone', function (val) {
  return val?val:'无'
})
...
Vue.prototype.$window.utils = {
  ...
  dict: {
     ...
     */
    get (type) {
      if (this.data[type] && this.data[type].hasOwnProperty('itemList')) {
    ...
    getName (val) {
      ...
      try {
        if (this.hasOwnProperty('nameList')) {
    ...
    key (type) {
      return this.data[type] || {getName: () => (''), getValue: () => ('')}
    },
    /**
     * 根据(字典识别码)获取(字典值)
     * @param val {string} 字典值
    ...
    getValue (val) {
      ...
      try {
        if (this.hasOwnProperty('dataList')) {
          return this.dataList[val].dictValue || ``
        }
      } catch (e) {
        console.warn('未找到字典名', val, this.dataList)
        return `未分类(${val})`
      }
    },
    init (data) {
      ...
      data && data.forEach(item => {
        ...
        result[key].dataList = lodash.keyBy(item.dictList, o => (o.dictKey))
        result[key].getValue = this.getValue
      })
      this.data = result
      return result
    },
...
Vue.filter('formatSeverity', function (val) {
  return Vue.prototype.$window.utils.dict.key('ticket_severity').getName(val)
})
Vue.filter('formatSolution', function (val) {
  return Vue.prototype.$window.utils.dict.key('ticket_resolve').getName(val)
})
Vue.filter('formatPriority', function (val) {
  return Vue.prototype.$window.utils.dict.key('ticket_priority').getName(val)
})
Vue.filter('ticketStatus', function (val) {
  return Vue.prototype.$window.utils.dict.key('ticket_type').getName(val)
})
