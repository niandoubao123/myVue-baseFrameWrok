...
export default {
    ...
    return {
      curProData: [
        ...
        {       
          ...
          labelOffset: [0, -24],
          lat: 30.245853, // 经度
          lng: 120.209947, // 纬度, 120.209947,30.245853
          state: 5
        },
        {       
          province: "云南省",
          ...
          labelOffset: [0, -24],
          lat: 24.881539, // 经度
          lng: 102.833722, // 纬度, 102.833722,24.881539
          state: 4
        },
        {       
          province: "陕西省",
      ...
      geoProvince: {
        '江苏省':[118.779088,32.0592],
        '云南省':[102.709372,25.046432],
        '陕西省':[108.948024,34.263161],
        '贵州省':[106.705251,26.600328],
      },
      // ------------高德地图参数开始-----------------
  ...
  methods: {
    ...
    initPage() {
      ...
      console.log('这是啥',markers,obj)
      // 绘画省份的点和数量   SimpleMarker   画点的
      AMapUI.loadUI(['overlay/SimpleMarker'], function (SimpleMarker) {
        ...
        for (let key in obj) {
          markers.push(
            new SimpleMarker({
              zIndex: 50,
              // iconLabel: '<div class="mapIcon" style="width:50px;height:20px;background:#fff;"><span class="mapIcon_title">' + key + '</span><span class="mapIcon_num">' + obj[key] + '</span></div>',
    ...
    // 绘制路径   PathSimplifier   画线/路径的
    setLine(data) {
      let map = this.amapManager.getMap()   //拿到目标对象
      AMapUI.loadUI(['misc/PathSimplifier'], function(PathSimplifier) {
        if (!PathSimplifier.supportCanvas) {
          alert('当前环境不支持 Canvas！');
          return;
        }
        var pathSimplifierIns = new PathSimplifier({   //绘制路径的方法   下面传值并调用了
            zIndex: 100,
            ...
            getHoverTitle: function(pathData, pathIndex, pointIndex) {   //当悬停到所画的线时候才会触发并显示的东西
              // console.log('应该是一样的吧')
        ...
        //测试结束
        // 设定数据源数组，并触发重新绘制。data为空时将清除显示内容。  自动执行？
        // pathSimplifierIns.setData([{
        //     name: '北京-莫斯科 线路',
        //     path: [
        //       [116.405289, 39.904987],
          ...
          if (item.lng && item.lat) {
            markers.push(
              new SimpleMarker({
                // 定位点的样式
                showPositionPoint: {
                  color: item['state'] == 5 || item['state'] == 4 ? color[item['state']] : 'rgba(255,255,255,0)',
                  radius: 10
                },
                // 自定义定位点,波纹感的圆点
                iconLabel: '<div class="example"><span class="dot colorStyle'+ item.state +'"></span></div>',              
                //直接设置html(需要以"<"开头并且以">"结尾)
                map: map,
                position: [item.lng, item.lat], // 经纬度
                // 定位点的label标签
                label: {
                    offset: item['labelOffset'] ? new AMap.Pixel(+item['labelOffset'][0], +item['labelOffset'][1]) : '', // 修改label相对于marker的位置
                    // offset: new AMap.Pixel(0,0), // 修改label相对于marker的位置
                    content: '<div class="makerLabel"><span class="mapIcon_title" style="color:rgb(9,21,42)">' + item.city + '</span></div>'
                }
              }),
            )
          }
...
<style lang="scss" scoped>
  ...
  .colorStyle3:after{
    box-shadow: 0 0 20px rgba(243,161,0,.3) outset;
  }
  .colorStyle4:after{
