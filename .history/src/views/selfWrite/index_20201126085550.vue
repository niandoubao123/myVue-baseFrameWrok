...
<script>
  ...
  import vmConfig from './frameWork/vmConfigHandler.js'
	export default {
    beforeCreate(){
			let methods = (what, ...param) => {
				let func = methodsSource[what] || methodsSource.prototype[what] // 目标函数，methodsSource可能是字面量对象，也可以是类
				if (!func) throw new Error('找不到 methods：' + what)
