<template>
    ...
    <div>
      ...
      <button @click="hereEvent()">测试</button>
    </div>
	</div>
</template>
...
<script>
  ...
  import vmConfig from './frameWork/vmConfigHandler.js'
    ...
    created(){
		...
		//(this.func->里面的this.methods('name')->去到框架的读取methodsSource中的方法,这个方法又经过更改上下文)
		//event里面的东西实际上并没有和vue里面的click进行实质的关联
		//例如：页面点击->去this里面找被挂到this上的event里面的方法->调用每个方法里面的this.methods(name)->这个方法又回去methods.js中匹配方法，操作数据（这里）
		/* data() {
